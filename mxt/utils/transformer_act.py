...
def discrete_autoregreesive_act(decoder, obs_rep, obs, batch_size, n_agent, action_dim, tpdv,
    ...
    shifted_action = torch.zeros(
        (batch_size, n_agent, action_dim + 1)).to(**tpdv)
    ...
    for i in range(n_agent):
        ...
        action = distri.probs.argmax(
            dim=-1) if deterministic else distri.sample()
        ...
        if i + 1 < n_agent:
            shifted_action[:, i + 1,
                           1:] = F.one_hot(action, num_classes=action_dim)
...
def continuous_autoregreesive_act(decoder, obs_rep, obs, batch_size, n_agent, action_dim, tpdv,
    ...
    output_action = torch.zeros(
        (batch_size, n_agent, action_dim), dtype=torch.float32)
    output_action_log = torch.zeros_like(output_action, dtype=torch.float32)
