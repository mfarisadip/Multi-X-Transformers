...
class Attention(nn.Module):
    def __init__(
        ...
        self.to_out = nn.Sequential(nn.Linear(
            out_dim, dim * 2, bias=False), nn.GLU()) if on_attn else nn.Linear(out_dim, dim, bias=False)
    ...
    def forward(
        ...
        b, n, _, h, talking_heads, head_scale, scale, device, has_context = * \
            x.shape, self.heads, self.talking_heads, self.head_scale, self.scale, x.device, exists(
                context)
        ...
        if self.scale_log_seq:
            num_tokens_attended = (dots > mask_value).sum(
                dim=-1, keepdims=True)
            log_seq_scale = torch.log(
                num_tokens_attended + 1) / self.scale_log_seq_denom
